...
def setup_logging(log_dir: str) -> str:
    ...
    log_file = os.path.join(log_dir, f"annul_log_{datetime.now().strftime('%Y%m%d')}.log")
    return log_file
...
# template spécifique à annul.py
OUTPUT_TEMPLATE = config.get(output_section, "annul")
...
def main() -> int:
    if len(sys.argv) < 2:
        msg = "Usage: python annul.py <fichier_source AC111 MAMT003>"
    ...
    try:
        ...
        # ---------- Lecture du contenu AC111 MAMT003 ----------
        colspecs = [
            (0, 2),
            (2, 9),
            (9, 44),
            (44, 79),
            (79, 87),   # DateEffetAnnulation
            (87, 122),
            (122, 157),
            (157, 161),
            (161, 301),
        ...
        df["DateEffetAnnulation"] = df["DateEffetAnnulation"].astype(str).str.strip()
        df["_date_eff"] = pd.to_datetime(df["DateEffetAnnulation"], format="%Y%m%d", errors="coerce").dt.normalize()
        ...
        nb_lignes = len(df)
        if nb_lignes == 0:
            msg = "Aucune ligne de mandat à traiter dans le fichier AC111 MAMT003."
            log_message(log_file, msg)
            print(msg)
            return 0
        ...
        # NB : pour MAMT003 (ANNUL), l'entête PARTNER n'est PAS paddée (cf. attendu PARTNER)
        header_row = [
            "PARTNER-MDT",
            "3",
            "0",
        ...
        # TOTAL_FIELDS ne sert qu'aux lignes détail (MAMT003 = 62 champs en détail)
        TOTAL_FIELDS = 63

        # ---------- Construction des lignes PARTNER ----------
        rows = []

        for _, r in df.iterrows():
            partner = ["" for _ in range(128)]
            ...
            # Date de révocation (champ 10 dans tes scripts)
            partner[10] = convert_yyyymmdd_to_iso_date(r.get("DateEffetAnnulation", ""))
